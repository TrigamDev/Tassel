...
public class TasselEnchantmentScreen extends HandledScreen<TasselEnchantmentScreenHandler> {
	// Elements
	private static final Identifier TEXTURE = Tassel.id("textures/gui/container/enchanting_table.png");
	private static final Identifier SCROLLER_TEXTURE = Identifier.ofVanilla("container/stonecutter/scroller");
	private static final Identifier SCROLLER_DISABLED_TEXTURE = Tassel.id("container/enchanting_table/scroller_disabled");

	// Tooltips
	private static final Text GEAR_SLOT_TOOLTIP = Text.translatable(
		Util.createTranslationKey("container", Tassel.id("enchanting_table.base_slot_description"))
	).formatted(Formatting.WHITE);
	private static final Text CATALYST_SLOT_TOOLTIP = Text.translatable(
		Util.createTranslationKey("container", Tassel.id("enchanting_table.catalyst_slot_description"))
	).formatted(Formatting.WHITE);
	...
	//  Initialization
	//
	public TasselEnchantmentScreen(TasselEnchantmentScreenHandler handler, PlayerInventory inventory, Text title) {
	...
	protected void drawBackground(DrawContext context, float delta, int mouseX, int mouseY) {
		...
		context.drawTexture(TEXTURE, origin[0], origin[1], 0f, 0f, this.backgroundWidth, this.backgroundHeight, 256, 256);
